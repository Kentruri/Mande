...
router.get('/servicios-activos', isLoggedInUser, async (req, res) => {
    const servicios = await (await pool.query('SELECT * FROM trabajador JOIN (SELECT * FROM servicio) AS S ON id_trabajador=trabajador_id WHERE usuario_id = $1 AND servicio_estado=1', [req.user.id_usuario])).rows;
