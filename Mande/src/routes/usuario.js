...
router.get('/contratar-trabajador/:trabajador_id/:trabajador_nombre/:nombre_labor/:servicio_descipcion', isLoggedInUser, async (req, res, done) => {
...
router.post('/pagar-servicio/:id_servicio/:id_pago/:trabajador_id', async (req, res, done) => {
    ...
    const numeros = await (await pool.query('SELECT trabajador_calificaciones, trabajador_trabajosHechos FROM trabajador WHERE id_trabajador=$1', [trabajador_id])).rows[0];
    const trabajador_calificaciones=parseInt(numeros.trabajador_calificaciones)+parseInt(calificacion);
    const trabajador_trabajosHechos=parseInt(numeros.trabajador_trabajoshechos);
    const trabajador_puntaje = (trabajador_calificaciones/trabajador_trabajosHechos).toFixed(1);
    await pool.query('UPDATE trabajador SET trabajador_puntaje=$1, trabajador_calificaciones=$2 WHERE id_trabajador=$3', [trabajador_puntaje, trabajador_calificaciones, trabajador_id]);
